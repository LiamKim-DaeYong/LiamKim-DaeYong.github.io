...
위의 코드를 보면 `MemberService`는 더 이상 **구현체에 의존하지 않고(DIP)** `MemberRepository`의 **변경(확장)이 발생해도 영향을 받지 않는다.(OCP)**  또한 `AppConfig`가 변경 된다고 할지라도 `MemberService`는 변경 하지 않아도 된다.
다시 말해 `MemberService`의 **변경의 사유는 오직 회원관리 로직의 변경 딱 하나만 남게 되었다.(SRP)**
