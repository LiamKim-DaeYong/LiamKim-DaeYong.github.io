...
draft: false
description: "트랜잭션 경계, 격리수준, 락 전략, MVCC를 중심으로 정합성을 지키는 구조를 사례 기반으로 정리한다."
...
다음 글에서는 SQL 실행 흐름(파싱 -> 최적화 -> 실행)과
`EXPLAIN (ANALYZE, BUFFERS)` 읽기를 중심으로,
"왜 느린지"를 감이 아니라 수치 근거로 설명하는 단계로 들어간다.
