...
draft: false
description: "인덱스가 있어도 느릴 수 있는 이유, SQL 실행 흐름, 실행계획 읽는 법을 실습 예시와 함께 정리한다."
...
후속 글에서는 인덱스 내부 구조(B-Tree, 복합/커버링 인덱스)와 조인 방식(Nested Loop / Hash Join / Merge Join)을 중심으로,
"언제 어떤 방식이 선택되는지"와 "실무에서 어떻게 읽어야 하는지"를 이어서 정리할 예정이다.
